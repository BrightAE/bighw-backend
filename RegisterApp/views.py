...
from .models import Account, MySession, MyData
...
def logon(request):
    ...
    if len(request.POST) != 4 or ('username' not in request.POST) or ('password' not in request.POST) \
            or ('mail' not in request.POST) or ('id' not in request.POST) \
            or min(len(request.POST['username']), len(request.POST['password'])) == 0\
            or min(len(request.POST['mail']), len(request.POST['id'])) == 0:
    ...
    items = Account.objects.all()
    for item in items:
        print(item.id, ' ', item.username, ' ', item.password, ' ', item.mail, ' ', item.student_id, ' ', item.authority, ' ',
              item.active)
    ...
    return JsonResponse({test.username: "the-user-name"})


def active(request, rand_str):
    if request.method != 'GET':
        return JsonResponse({"error": "require GET"})
    saved_account = Account.objects.filter(rand_str=rand_str)
    if not saved_account.exists():
        return JsonResponse({'error:': "no such RandString"})
    saved_account = Account.objects.get(rand_str=rand_str)
